...
export function BlockContainer({ block, isEditing, onUpdate, onDelete, onAddBefore, onAddAfter, onDuplicate, onPasteBlock, onFocus }: Props) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`block-container rounded-lg mb-2 relative group ${isEditing ? 'bg-white border border-gray-100 p-3' : block.type === 'image' ? (block.isDoubleSided ? 'px-4 py-4' : 'px-4 py-0') : 'px-4 py-4'}`}
